...
Nomad 项目使用 `next-openapi-gen` 插件自动生成 OpenAPI 3.0 规范文档，提供类型安全的 API 开发体验。所有 API 接口都遵循统一的响应格式规范，确保前后端交互的一致性。
...
- **响应格式 Schema**: `src/types/api/response.ts` - 统一的响应格式定义
- **业务数据 Schema**: `src/types/api/` 目录下按模块组织（如 `users.ts`, `flights.ts`, `auth.ts`）
...
import { userSchema, type User } from "@/types/api/users";
...
export async function GET() {
  ...
  // 使用 ApiResponse 工具类返回标准格式
  return ApiResponse.success(validatedUsers);
}
```
...
import { z } from "zod";
import { ApiResponse } from "@/lib/utils/api-response";
import { createUserRequestSchema, userSchema } from "@/types/api/users";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 使用 ApiResponse 工具类返回标准格式（201 Created）
    return ApiResponse.success(validatedUser, 201);
  } catch (error) {
...
- **响应格式 Schema**: 统一放在 `src/types/api/response.ts`
- **业务数据 Schema**: 放在 `src/types/api/` 目录下按功能模块组织（如 `users.ts`, `flights.ts`, `auth.ts`）
- 使用描述性的 schema 名称
- 通过 `index.ts` 文件统一导出，简化导入路径

### 2. 错误处理

- 统一使用 Zod 进行数据验证
- 使用 `ApiResponse` 工具类确保响应格式一致
- 明确区分错误类型和状态码：
  - **数据格式/类型错误** → 400 Bad Request
...
这个文件已经在项目中创建，包含了所有标准响应格式的定义。主要包括：

- `responseMetaSchema`: 响应元数据
- `errorDetailSchema`: 错误详情
- `createSuccessResponseSchema()`: 成功响应泛型函数
- `errorResponseSchema`: 错误响应
- `paginationMetaSchema`: 分页信息
- `createPaginatedResponseSchema()`: 分页响应泛型函数
- `paginationQuerySchema`: 分页查询参数验证

所有相关的 TypeScript 类型都通过 `z.infer<>` 自动推导生成。

**使用示例：**

```typescript
// src/types/api/flights.ts
import { z } from "zod";
import { createSuccessResponseSchema } from "./response";
...
// 创建特定响应的 Schema
export const flightResponseSchema = createSuccessResponseSchema(flightSchema);
...
`requestId` 由 `ApiResponse` 工具类**自动生成**，开发者无需手动传入。同时为了日志记录和微服务调用链追踪，也支持手动传入：

```typescript
// 1. 自动生成 requestId（默认行为）
return ApiResponse.success(data);
// 响应中会自动包含：requestId: "req_k8pQxJ2mN5vL9wRt"

// 2. 手动传入 requestId（用于调用链追踪）
const customRequestId =
  request.headers.get("x-request-id") || generateRequestId();
return ApiResponse.success(data, 200, { requestId: customRequestId });
```
...
```typescript
// 使用项目中已定义的分页参数 Schema
import { paginationQuerySchema } from "@/types/api/response";
import { ApiResponse } from "@/lib/utils/api-response";
...
export async function GET(request: NextRequest) {
  ...
  const items = await getItems(query.page, query.pageSize);
...
DELETE 方法应返回 204 No Content，且**不应包含响应体**。这与标准响应格式不同：
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // 错误情况仍然返回标准错误格式
    return ApiResponse.notFound("BUSINESS_RESOURCE_NOT_FOUND", "资源未找到");
  }
}
```
...
| 状态码 | 说明                 | 使用场景                                                |
| ------ | -------------------- | ------------------------------------------------------- |
| 400    | Bad Request          | **数据格式/类型错误**（如JSON格式错误、字段类型不匹配） |
| 401    | Unauthorized         | 未授权，需要认证                                        |
| 403    | Forbidden            | 已认证但无权限                                          |
| 404    | Not Found            | 资源不存在                                              |
| 409    | Conflict             | 资源冲突（如重复创建）                                  |
| 422    | Unprocessable Entity | **业务规则违反**（如余额不足、状态不允许操作）          |
...
项目提供了 `ApiResponse` 工具类来简化响应构建，实现位置：`src/lib/utils/api-response.ts`。
