...
- [ ] 5.0.1 Read complete proposal: `openspec show update-user-menu-display`
...
- [ ] 5.3.1 更新导入语句
  - [ ] 添加 `ChevronDown` from `lucide-react`
  - [ ] 添加 `Link` from `next/link`
- [ ] 5.3.2 修改收起状态(第 66-68 行)
  - [ ] 将 `{session.user.name || "Anonymous"}` 改为 `"尊敬的用户"`
  - [ ] 在文本后添加 ChevronDown 图标
  - [ ] 图标样式:`size-3.5 text-muted-foreground`
  - [ ] 图标添加 `hidden md:inline-block` 响应式类
- [ ] 5.3.3 修改展开状态(第 82-87 行)
  - [ ] 将 Link 的文本从 `尊敬的{session.user.name || "Anonymous"}` 改为 `"尊敬的用户"`
- [ ] 5.3.4 将触发区域包装为 Link
  - [ ] 将 HoverCardTrigger 内的 `<div>` 改为 `<Link href="/home">`
  - [ ] 保持现有的 `flex items-center gap-2 cursor-pointer` 类
  - [ ] 确保 `asChild` prop 保留在 HoverCardTrigger 上
- [ ] 5.3.5 手动测试
  - [ ] 点击 UserMenu 跳转到 /home
  - [ ] 悬浮 UserMenu 显示下拉菜单
  - [ ] 检查不同屏幕尺寸(响应式)
...
- [ ] 5.4.1 创建 `src/stories/user-menu.stories.tsx`
- [ ] 5.4.2 导入必要的类型和组件
  - [ ] `Meta`, `StoryObj` from `@storybook/nextjs-vite`
  - [ ] `UserMenu` from `@/components/common/user-menu`
- [ ] 5.4.3 定义 meta 配置
  - [ ] title: `"Common/UserMenu"`
  - [ ] component: `UserMenu`
  - [ ] parameters: `{ layout: "centered" }`
- [ ] 5.4.4 创建 LoggedIn Story
  - [ ] Mock `authClient.useSession()` 返回 mock session
  - [ ] Session 数据:name, email, image
  - [ ] 渲染 UserMenu 组件
- [ ] 5.4.5 (可选)创建其他 Story
  - [ ] Loading Story: isPending = true
  - [ ] NotLoggedIn Story: session = null
- [ ] 5.4.6 测试 Storybook
  - [ ] 运行 `pnpm storybook`
  - [ ] 验证 Story 正常渲染
  - [ ] 检查交互功能(悬浮展开)
...
- [ ] 5.5.1 Run unit tests: `pnpm test`
- [ ] 5.5.2 Run full test suite: `pnpm test:run`
- [ ] 5.5.3 Run E2E tests: `pnpm e2e`
- [ ] 5.5.4 All tests should pass now (green phase of TDD)
- [ ] 5.5.5 Fix any failing tests
- [ ] 5.5.6 Verify test coverage: `pnpm test:coverage` (target: ≥70%)
...
- [ ] 6.3.1 Replace any `any` types with proper types
  - [ ] 检查 Storybook mock 是否使用正确的 session 类型
...
- [ ] ✅ Test coverage ≥ 70% for new code (if applicable)
- [ ] ✅ Linting passing (`pnpm lint`)
- [ ] ✅ Type checking passing (`tsc --noEmit`)
- [ ] ✅ Code formatted (`pnpm format`)
- [ ] ✅ No code duplication
- [ ] ✅ Good naming conventions followed
- [ ] ✅ Complex logic well-commented (N/A for this simple change)

### Implementation
- [ ] ✅ UserMenu 组件文本更新为"尊敬的用户"
- [ ] ✅ ChevronDown 图标添加完成,样式正确
- [ ] ✅ Link 包装实现,点击跳转到 /home
- [ ] ✅ 悬浮展开功能仍正常工作
- [ ] ✅ Storybook 文档创建完成
- [ ] ✅ 响应式显示正确(不同屏幕尺寸)
...
### Documentation
- [ ] ✅ Component JSDoc 注释添加
- [ ] ✅ Storybook 文档完整
- [ ] ✅ Requirements documentation reviewed
- [ ] ✅ User documentation reviewed (if applicable)
...
3. ✅ Tests written and passing (unit + E2E)
...
- Update specification with changes (if user-interface spec exists)
- Archive change proposal: `openspec archive update-user-menu-display`
