...
**All feature changes MUST follow this 6-phase workflow:**
...
1. **Read Requirements Documents**
   - Review `content/docs/requirements/functional-requirements/[module].mdx`
   - Identify affected requirement sections
   - Document requirement IDs in proposal

2. **Clarify Ambiguities**
   - Ask 1-2 clarifying questions if requirements are unclear
   - Confirm scope and acceptance criteria

**Output:** Clear understanding of functional requirements referenced in `proposal.md`
...
1. **Analyze Current Implementation**
   - Check existing API routes: `src/app/api/[route]/route.ts`
   - Check existing Server Actions: `src/app/[route]/actions.ts`
   - Review related Drizzle schemas: `src/lib/schema/`

2. **Make Reuse vs New Decision**
   - **Reuse existing:** Extend current endpoint/action if compatible
   - **Breaking change:** Version API or create new endpoint
   - **New feature:** Create new endpoint/action

3. **Document API Design**
   - Create/update `content/docs/technical-design/backend/[feature]-api.mdx`
   - Include: endpoint paths/action names, request/response schemas (Zod), error codes
   - Document authentication requirements

**Output:** Technical design documented in `design.md` and API documentation

#### Phase 3: Update API Specification

**Goal:** Maintain API documentation in sync

1. **Update OpenAPI Spec** (if using REST APIs)
   - Run `pnpm api:generate` after implementation
   - Verify generated spec is correct

2. **Add JSDoc Comments** (for Server Actions)
   - Document function parameters and return types
   - Include usage examples

**Output:** Up-to-date API documentation

#### Phase 4: Test Design

**Goal:** Define test coverage BEFORE implementation

1. **Write Test Plan**
   - Based on requirements and API design
   - Include test scenarios for success and error cases
   - Define test data fixtures using `@faker-js/faker`

2. **Plan Test Levels**
   - **Unit tests:** Business logic, utility functions (`*.test.ts` co-located)
   - **Integration tests:** API + Database interactions
   - **E2E tests:** User flows with Playwright (`tests/`)

**Output:** Test scenarios documented in `tasks.md` before implementation tasks

#### Phase 5: Implementation

**Goal:** Build the feature following test-driven approach
...
3. **API/Server Actions**
   - Implement endpoints or Server Actions
   - Add Zod validation
...
**Goal:** Verify correctness and update documentation

**Testing Sequence (MUST follow order):**

1. **Unit Tests**
   - Write tests co-located with source: `*.test.ts`
   - Run: `pnpm test`
...
4. **Full Test Suite**
   - Run: `pnpm test:run && pnpm e2e`
   - All tests must pass

**Documentation Updates:**

1. Update API documentation (`content/docs/technical-design/backend/`)
2. Update user guides (`content/docs/`)
3. Add inline code comments for complex logic
4. Update CHANGELOG.md (if applicable)

**Output:** All tests passing, documentation updated
...
AI assistants MUST NOT start Phase 5 (Implementation) without completing Phases 1-4 and receiving explicit user approval.
