...
export interface FlightSummaryCardFlightProps {
  id: string;
...
export interface FlightSummaryCardProps {
  outboundFlight: FlightSummaryCardFlightProps | null;
  inboundFlight?: FlightSummaryCardFlightProps | null;
...
interface FlightSegmentProps {
  flight: FlightSummaryCardFlightProps;
...
export function FlightSummaryCard({
  ...
  // Calculate prices
  const outboundPrice = Number.parseFloat(outboundFlight.price);
  const inboundPrice = inboundFlight
    ? Number.parseFloat(inboundFlight.price)
    : 0;
  const totalPricePerTicket = outboundPrice + inboundPrice;
  const totalPrice = totalPricePerTicket * passengerCount;
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="font-medium">
              {formatCurrency(outboundFlight.price)}
            </span>
          </div>
          ...
          {inboundFlight && (
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="font-medium">
                {formatCurrency(inboundFlight.price)}
              </span>
            </div>
          )}
