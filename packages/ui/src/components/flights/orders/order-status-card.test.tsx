...
import type { OrderStatusCardData } from "./order-status-card";
...
describe("OrderStatusCard Component", () => {
  ...
  const mockOnRequestRefund = vi.fn();
  ...
  describe("Rendering", () => {
    ...
    it("should render order status card with order number", () => {
      ...
      render(<OrderStatusCard data={data} timeLeft={850} />);
  ...
  describe("Action Buttons", () => {
    ...
    it('should render "Cancel Order" button when callback provided', () => {
      const data = createMockStatusData({ status: "PENDING_PAYMENT" });
      render(
        <OrderStatusCard
          data={data}
      ...
      const button = screen.getByRole("button", { name: "取消订单" });
      expect(button).toBeInTheDocument();
    });
    ...
    it('should render "Request Refund" button when canRefund is true', () => {
      const data = createMockStatusData({ status: "CONFIRMED" });
      render(
        <OrderStatusCard
          data={data}
          onRequestRefund={mockOnRequestRefund}
          canRefund={true}
        />
      );

      const button = screen.getByRole("button", { name: "申请退款" });
      expect(button).toBeInTheDocument();
    });

    it('should not render "Request Refund" button when canRefund is false', () => {
      const data = createMockStatusData({ status: "CONFIRMED" });
      render(
        <OrderStatusCard
          data={data}
      ...
      expect(
        screen.queryByRole("button", { name: "申请退款" })
    ...
    it('should call onCancelOrder when "Cancel Order" button is clicked', async () => {
      const user = userEvent.setup();
      const data = createMockStatusData({ status: "PENDING_PAYMENT" });
      render(
        <OrderStatusCard
    ...
    it('should call onRequestRefund when "Request Refund" button is clicked', async () => {
      const user = userEvent.setup();
      const data = createMockStatusData({ status: "CONFIRMED" });
      render(
        <OrderStatusCard
      ...
      expect(mockOnRequestRefund).toHaveBeenCalledTimes(1);
    });
  ...
  describe("Countdown Timer Display", () => {
    it("should display countdown timer with provided timeLeft", () => {
      const data = createMockStatusData({
        status: "PENDING_PAYMENT",
      });
      render(<OrderStatusCard data={data} timeLeft={850} />);

      // Check for countdown format (MM:SS) - 850 seconds = 14:10
      expect(screen.getByText("14:10")).toBeInTheDocument();
    });

    it("should show expiration message when timeLeft is 0", () => {
      const data = createMockStatusData({
        status: "PENDING_PAYMENT",
      });
      render(<OrderStatusCard data={data} timeLeft={0} />);
    ...
    it("should not display payment button when timeLeft is 0", () => {
      const data = createMockStatusData({
        status: "PENDING_PAYMENT",
      });
      render(
    ...
    it("should display warning color when timeLeft is low", () => {
      const data = createMockStatusData({
        status: "PENDING_PAYMENT",
      });
      render(<OrderStatusCard data={data} timeLeft={45} />);

      const countdown = screen.getByText("00:45");
      expect(countdown).toHaveClass("text-destructive");
    });

    it("should display normal color when timeLeft is sufficient", () => {
      const data = createMockStatusData({
        status: "PENDING_PAYMENT",
      });
      render(<OrderStatusCard data={data} timeLeft={850} />);

      const countdown = screen.getByText("14:10");
      expect(countdown).toHaveClass("text-secondary");
    });
  });
